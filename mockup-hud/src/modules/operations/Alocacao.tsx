...
import { ArrowRight, Plus, Trash2, Truck, Edit2, CheckCircle, XCircle, RefreshCw } from 'lucide-react'
    ...
    <div className="space-y-4 h-full overflow-y-auto pb-20">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={()=>{setFluxoForm(emptyFluxo);setEditingFluxo(null);setOpenFluxo(true)}} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-mono uppercase text-brand-400 bg-brand-600/10 border border-brand-600/30 rounded-md hover:shadow-glow-sm transition-all"><Plus className="w-3 h-3"/>Novo Fluxo</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="bg-hud-panel border border-hud-border rounded-lg p-3 text-center">
            ...
            <div key={fluxo.id} className={`bg-hud-panel border rounded-xl overflow-hidden transition-all ${isAtivo?'border-hud-border':'border-hud-border/40 opacity-60'}`}>
              ...
              <div className="px-5 py-3 flex items-center justify-between border-b border-hud-border/30">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={()=>{setAlocarFluxo(fluxo);setOpenAlocar(true)}} className="flex items-center gap-1 px-2 py-1 text-[9px] font-mono uppercase text-ok bg-ok/10 border border-ok/20 rounded hover:shadow-glow-sm transition-all" title="Alocar equipamento"><Plus className="w-3 h-3"/>Alocar</button>
                  ...
                  <button onClick={()=>{setFluxoForm({origem_area:fluxo.origem_area,origem_subarea:fluxo.origem_subarea,material:fluxo.material,destino_area:fluxo.destino_area,destino_subarea:fluxo.destino_subarea,material_destino:fluxo.material_destino});setEditingFluxo(fluxo);setOpenFluxo(true)}} className="p-1 text-dim hover:text-gray-300"><Edit2 className="w-3.5 h-3.5"/></button>
                  <button onClick={()=>setDelFluxo(fluxo)} className="p-1 text-dim hover:text-crit"><Trash2 className="w-3.5 h-3.5"/></button>
                </div>
              </div>
