...
import { Layers, Eye, EyeOff, Search, X, Truck, ChevronRight, Fuel, Clock, Gauge, Activity, MapPin, Radio, Thermometer, Zap, TrendingUp, AlertTriangle, CheckCircle2, BarChart3, ArrowUp, ArrowDown } from 'lucide-react'
...
export default function Mapa() {
  ...
  return (
    <div className="h-full relative flex rounded-xl overflow-hidden border border-hud-border">
      
      {/* ═══ LEFT DRAWER — EQUIPMENT LIST ═══ */}
      <div className={`relative z-[1000] flex flex-col transition-all duration-300 ${drawerOpen?'w-80':'w-0'} bg-hud-panel/95 backdrop-blur-xl border-r border-hud-border overflow-hidden`}>
        {/* Search */}
        <div className="p-3 border-b border-hud-border/50">
          <div className="relative">
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar equipamento, modelo, operador..." className="w-full pl-9 pr-8 py-2 bg-hud-bg border border-hud-border rounded-lg text-xs text-gray-200 placeholder-dim/60 focus:border-brand-600/50 focus:outline-none transition-colors" />
      ...
      <button onClick={()=>setDrawerOpen(!drawerOpen)} className="absolute top-4 left-4 z-[1001] p-2 bg-hud-panel/95 backdrop-blur-md border border-hud-border rounded-lg shadow-panel text-dim hover:text-brand-400 transition-colors" style={{left: drawerOpen?'330px':'16px'}}>
      ...
      {/* ═══ RIGHT PANEL — EQUIPMENT SNAPSHOT ═══ */}
      {selectedEquip && snap && (
        <div className="w-[380px] z-[1000] bg-hud-panel/95 backdrop-blur-xl border-l border-hud-border overflow-y-auto scrollbar-thin animate-slideIn">
          {/* Header */}
          <div className="sticky top-0 z-10 bg-hud-panel/95 backdrop-blur-xl border-b border-hud-border/50 px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={()=>setSelectedEquip(null)} className="p-1 text-dim hover:text-gray-300"><X className="w-4 h-4"/></button>
            </div>
